open Ast
open Bytecode

type lit = 
    Int of int
  | String of string
  | Bool of bool

let string_of_lit = function
    Int(i) -> string_of_int i
  | String(s) -> s
  | Bool(b) -> string_of_bool b

let implode lst =
  let res = String.create (List.length lst) in
  let rec imp i = function
    | [] -> res
    | c :: lst -> res.[i] <- c; imp (i + 1) lst in
    imp 0 lst

let execute_prog prog debug_flag =
  let stack = Array.make 1024 (Int 0)
  and canv_env = Array.make 100 []
  and globals = Array.make prog.num_globals (Int 0) 
  and debug s =
   if debug_flag then print_endline s
  in 
    debug ("DEBUG: num_globals is " ^ string_of_int prog.num_globals);
    try
      let rec exec fp sp pc = match prog.text.(pc) with 
          Lit i -> 
            stack.(sp) <- Int i; 
            debug ("DEBUG: Lit " ^ string_of_int i);
            exec fp (sp+1) (pc+1)
        | Stl s -> 
            stack.(sp) <- String s; 
            debug ("DEBUG: Stl " ^ s);
            exec fp (sp+1) (pc+1)
        | Boo b -> 
            stack.(sp) <- Bool b; 
            debug ("DEBUG: Boo " ^ string_of_bool b);
            exec fp (sp+1) (pc+1)
        | Drp -> 
            debug ("DEBUG: Drp ");
            exec fp (sp-1) (pc+1)
        | Bin op ->
            debug ("DEBUG: Bin ");
            let op1 = stack.(sp-2) and op2 = stack.(sp-1) in
              (stack.(sp-2) <- 
               (let boolean b = if b then 1 else 0 
                and bool_of_int i = if i > 0 then true else false
                in 
                  (match (op1, op2) with
                       (Int(i), Int(j)) ->
                         (match op with
                              Plus ->  
                                debug("Bin +: i=" ^ string_of_int i ^ " j=" ^ string_of_int j);
                                Int (i + j)
                            | Minus ->
                                debug("Bin -: i=" ^ string_of_int i ^ " j=" ^ string_of_int j);
                                Int (i - j)
                            | Times ->
                                debug("Bin *: i=" ^ string_of_int i ^ " j=" ^ string_of_int j);
                                Int (i * j)
                            | Divide ->
                                debug("Bin /: i=" ^ string_of_int i ^ " j=" ^ string_of_int j);
                                Int (i / j)
                            | Mod ->
                                debug("Bin mod: i=" ^ string_of_int i ^ " j=" ^ string_of_int j);
                                Int (i mod j)
                            | Eq ->
                                debug("Bin eq: i=" ^ string_of_int i ^ " j=" ^ string_of_int j);
                                Int (boolean (i = j))
                            | Neq        -> 
                                debug("Bin neq: i=" ^ string_of_int i ^ " j=" ^ string_of_int j);
                                Int (boolean (i != j))
                            | Lt         -> 
                                debug("Bin <: i=" ^ string_of_int i ^ " j=" ^ string_of_int j);
                                Int (boolean (i < j))
                            | Gt         -> 
                                debug("Bin >: i=" ^ string_of_int i ^ " j=" ^ string_of_int j);
                                Int (boolean (i > j))
                            | Leq        -> 
                                debug("Bin <=: i=" ^ string_of_int i ^ " j=" ^ string_of_int j);
                                Int (boolean (i <= j))
                            | Geq        -> 
                                debug("Bin >=: i=" ^ string_of_int i ^ " j=" ^ string_of_int j);
                                Int (boolean (i >= j))
                            | Or         -> 
                                debug("Bin ||: i=" ^ string_of_int i ^ " j=" ^ string_of_int j);
                                Int (boolean ((bool_of_int i) || (bool_of_int j)))
                            | And        -> 
                                debug("Bin &&: i=" ^ string_of_int i ^ " j=" ^ string_of_int j);
                                Int (boolean ((bool_of_int i) && (bool_of_int j)))
                            | Mask       -> Int 1 (* need to do *)  )

                     | (_, _) ->
                         raise (Failure ("Operator must take integer operands."))
                  ))); 
                  exec fp (sp-1) (pc+1)
        | Lod i -> 
            stack.(sp) <- globals.(i); 
            debug ("DEBUG: Lod " ^ string_of_int i ^ " Global=" ^ string_of_lit globals.(i)); 
            exec fp (sp+1) (pc+1) 
        | Str i -> 
            globals.(i) <- stack.(sp-1); 
            debug ("DEBUG: Str " ^ string_of_int i); 
            exec fp sp (pc+1) 
        | Lfp i -> 
            stack.(sp) <- stack.(fp+i);
            debug ("DEBUG: Lfp " ^ string_of_int i);
            exec fp (sp+1) (pc+1) 
        | Sfp i -> 
            stack.(fp+i) <- stack.(sp-1); 
            debug ("DEBUG: Sfp " ^ string_of_int i);
            exec fp sp (pc+1) 
        (* here Jsr -1, refers to OutputC functionality *)
        | Jsr(-1) ->
            debug ("Jsr -1");
            print_endline (string_of_lit stack.(sp-1));
            exec fp sp (pc+1)
        | Jsr i -> 
            stack.(sp) <- Int (pc + 1); 
            debug ("DEBUG: Jsr " ^ string_of_int i);
            exec fp (sp+1) i
        | Ent i -> 
            stack.(sp) <- Int (fp); 
            debug ("DEBUG: Ent " ^ string_of_int i);
            exec sp (sp+i+1) (pc+1) 
        | Rts i ->
            let new_fp = match stack.(fp) with
                Int(k) -> k
            and new_pc = match stack.(fp-1) with
                Int(k) -> k 
            in
              stack.(fp-i-1) <- stack.(sp-1);
              debug ("DEBUG: Rts " ^ string_of_int i);
              exec new_fp (fp-i) new_pc 
        | Beq i -> 
            debug ("DEBUG: Beq " ^ string_of_int i);
            exec fp (sp-1)
              (pc + 
               if (match stack.(sp-1) with
                  Int(k) -> k) = 0 
               then i 
               else 1) 
        | Bne i -> 
            debug ("DEBUG: Bne " ^ string_of_int i);
            exec fp (sp-1)
              (pc + 
               if (match stack.(sp-1) with 
                   Int(k) -> k) != 0 
               then i 
               else 1) 
        | Bra i -> 
            debug ("DEBUG: Bra " ^ string_of_int i);
            exec fp sp (pc+i)
        | Lcv i -> ()
        | Hlt -> ()
      in exec 0 0 0 
    with e -> (* catch all exceptions *)
      Printf.eprintf "Unexpected exception: %s\n" (Printexc.to_string e);

